...
if (empty($DatosFormulario)) {
    ...
    // armamos el ID del Plan: CodCarrera + "P" + Nro revision
    $DatosFormulario["id"] = $DatosFormulario["idCarrera"]."P".$DatosFormulario["id"];
    // usado para volver a las revisiones del plan
    $idCarrera = "?id=".$DatosFormulario["idCarrera"];
    
    try {
        $consulta = $ManejadorPlan->alta($DatosFormulario);
    } catch (Exception $e) {
        $error = $e->getMessage();
    }
            ...
            <div class="container">
                ...
                <div class="card">
                    <div class="card-header">
                        <h3>Crear Nueva Revisi&oacute;n de Plan</h3>
                    </div>
                    <div class="card-body">
    <?php if ($consulta) { ?>
