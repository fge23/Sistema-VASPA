...
//    }
    function redireccionaIndex() {
        if (self::verificaPermiso("Profesor")) {
            $this->ubicacion = "http://localhost/uargflow/php";
            header("Location: {$this->ubicacion}");
        } else if (self::verificaPermiso("SA")) {
            $this->ubicacion = "http://localhost/uargflow/app/programa/PanelSA.php";
            header("Location: {$this->ubicacion}");
        }
    }
